...
Stream<RemoteEvent> subscribe() async* {
  var grpcClient = await setup();
  var subscription = grpcClient.subscribe(['notification'],
      registeredTypeRequired: true, managed: false);
  yield* subscription.eventStream;
}
      ...
      body: Center(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            StreamBuilder<RemoteEvent>(
              stream: stream,
              builder:
                  (BuildContext context, AsyncSnapshot<RemoteEvent> snapshot) {
                if (snapshot.hasError) return Text('Error: ${snapshot.error}');
                if (snapshot.hasData) event = snapshot.data;
                return Text('event = ${event?.id}');
              },
            )
          ],
        ),
